...
namespace WpfSimpleListBoxDrag
{
	public class HoverInteractor : DependencyObject
	{
		public static readonly DependencyProperty UseHoverProperty = DependencyProperty
			.RegisterAttached("UseHover", typeof(bool), typeof(HoverInteractor), new PropertyMetadata(false, OnUseHoverChanged));

		private static readonly DependencyProperty AttachedAdornerProperty = DependencyProperty
			.Register("AttachedAdorner", typeof(AdornerInfo), typeof(HoverInteractor));
		...
		private static void OnUseHoverChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
		{
		...
		private static void ListBox_MouseMove(object sender, MouseEventArgs e)
		{
			...
			ListBox lb = sender as ListBox;
			ListBoxItem item = lb.ContainerFromElement(e.OriginalSource as Visual) as ListBoxItem;
			if (item == null)
			{
				return;
			}
			...
			adorner.Container.ContentTemplate = item.FindResource("AdornerTemplate") as DataTemplate;
